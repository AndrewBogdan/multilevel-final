...
lm_data <- data %>%
  mutate(
    ...
    salary_scaled = replace_na(log2(ifelse(salary < 150000 & salary > 20000, salary, NA) / 60000), 0),
...
model <- lmer(
  ...
  cache("cache/model_full.rds", use_cache = TRUE)
...
data %>%
  mutate(salary_scaled = log2(ifelse(salary < 150000 & salary > 20000, salary, NA) / 60000)) %>%
  select(salary_scaled) %>%
