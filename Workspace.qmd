...
model <-  lmer(job_satis ~ (1 | pair_id), data = data)
...
model <- cache(
  expression = lmer(job_satis ~ (1 | major) + (1 | job_type), data = data %>% filter(survey_year == 2013)),
...
cl_model <- ordinal::clmm(
  ...
  # cache("cache/model_clmm.rds", use_cache = FALSE)
